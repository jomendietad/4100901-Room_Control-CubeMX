...
//       ring_buffer_write(&ring_buffer, uart2_rx_data);
//     }
// }
...
int main(void)
{
  ...
  // HAL_UART_Receive_IT(&huart2, &uart2_rx_data, 1);
  ring_buffer_write(&ring_buffer, 'H');
  ring_buffer_write(&ring_buffer, 'e');
  ring_buffer_write(&ring_buffer, 'l');
  ring_buffer_write(&ring_buffer, 'l');
  ring_buffer_write(&ring_buffer, 'o');
  ring_buffer_write(&ring_buffer, ' '); // Agrega un espacio al buffer
  ring_buffer_write(&ring_buffer, 'W');
  ring_buffer_write(&ring_buffer, 'o');
  ring_buffer_write(&ring_buffer, 'r');
  ring_buffer_write(&ring_buffer, 'l');
  ring_buffer_write(&ring_buffer, 'd');
  ring_buffer_write(&ring_buffer, '!');
  ring_buffer_write(&ring_buffer, '\n');
  ring_buffer_write(&ring_buffer, '\r');
  ring_buffer_write(&ring_buffer, ' '); // Agrega un espacio al buffer
  ring_buffer_write(&ring_buffer, 'H');
  ...
  while (1)
  {
    led_toggle(&led1); // Alterna el estado del LED
    HAL_Delay(500);
    // if (ring_buffer_count(&ring_buffer) >= 5) {
    //   // If there are at least 5 bytes in the ring buffer, read and process them
    //   for (int i = 0; i < 5; i++) {
    //     if (ring_buffer_read(&ring_buffer, &uart2_rx_data)) {
